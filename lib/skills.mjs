 ...
 *
 * The SKILL.md frontmatter is the SINGLE OWNER of the skill's description on
 * every runtime: Claude Code and ZCode read that file directly, so this row
 * parses the same frontmatter instead of carrying its own copy that can drift.
 * The constants below are a fallback for a present-but-unparseable file only.
 *
...
/** The `description:` line of the shipped SKILL.md frontmatter, or undefined. */
const frontmatterDescription = (text) => {
  const fm = text.match(/^---\r?\n([\s\S]*?)\r?\n---/)
  if (!fm) return undefined
  const line = (fm[1].match(/^description:[ \t]*(.+)$/m) || [])[1]
...
export function apply(ctx, config) {
  ...
  const owned = frontmatterDescription(body)
  const description = owned || FALLBACK_DESCRIPTION
  const whenToUse = owned || FALLBACK_WHEN_TO_USE
  if (!owned) {
    log.warn('formalswarm-skills: SKILL.md frontmatter has no description line, using the built-in fallback strings')
  }
